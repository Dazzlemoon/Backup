...
from matgl.models import TensorNet_LR, CHGNet_LR, CACE_LR, CHGNet
...
dataset_path = '/home/zhongpc/Project/matgl_long-range/train_CHGNet/dataset/dipep_4.5'
...
model = CHGNet(cutoff = 4.5, num_blocks = 3)
...
for ii in range(num_restart):
    ...
    lit_model = myPotentialLightningModule(
        ...
        # les_params = {'les_dl': 2.0, 'les_sigma': 1.0, 'les_norm_factor': 1.0},
    )


    if ii == (num_restart -1): # last restart

        # lit_model.patience = 5
...
lit_model.energy_weight = 1000.0
...
# Resume from the best checkpoint
checkpoint_path = "./phase3_last_checkpoint.ckpt"  # Replace with your checkpoint path
trainer.fit(
    model=lit_model, 
    train_dataloaders=train_loader, 
    val_dataloaders=val_loader,
...
trainer.save_checkpoint("./phase4_last_checkpoint.ckpt")
torch.save(lit_model, "./phase4_lit_model.pth")