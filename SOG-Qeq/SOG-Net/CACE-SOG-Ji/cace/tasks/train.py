...
class TrainingTask(nn.Module):
    def __init__(self, 
        ...
        #self.loss_list=[]
        self.min_loss = 10000
        # 能量：保留较松的初值，便于早期出现“最优”检查点
        self.train_MAE_e = 2.5e-3#1e-3#
        self.train_RMSE_e = 2.5e-3#1e-3#
        self.test_MAE_e = 1e-3#1e-3#
        self.test_RMSE_e = 1e-3#1e-3#
        # 力：原先 train/test 均设为 1e-3，要求第一次保存时 train 与 val 力 MAE 都 <0.001，
        # 对多数体系过严，导致 min_mae_f / min_rmse_f 永远不触发。改为 +inf 表示“尚无记录”，
        # 与 best_val_loss 一致，首次验证后即可按双指标改进保存。
