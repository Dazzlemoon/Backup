...
class SOGPotential(nn.Module):
    ...
    def _ensure_legacy_sog_attrs(self, positions: torch.Tensor) -> None:
        """旧 checkpoint 可能未包含 __init__ 中的部分属性，反序列化后访问会报错。"""
        dev, dt = positions.device, positions.dtype
        if not hasattr(self, "Periodic"):
            self.Periodic = False
        if not hasattr(self, "ene_factor"):
            self.ene_factor = torch.tensor(0.0, device=dev, dtype=dt)
        elif isinstance(self.ene_factor, torch.Tensor):
            self.ene_factor = self.ene_factor.to(device=dev, dtype=dt)
        if not hasattr(self, "norm_factor"):
            self.norm_factor = torch.tensor(1.0, device=dev, dtype=dt)
        elif isinstance(self.norm_factor, torch.Tensor):
            self.norm_factor = self.norm_factor.to(device=dev, dtype=dt)
        if not hasattr(self, "sigma"):
            self.sigma = 1.0
        if not hasattr(self, "sigma_sq_half"):
            self.sigma_sq_half = float(self.sigma) ** 2 / 2.0
        if not hasattr(self, "twopi"):
            self.twopi = 2.0 * torch.pi
        if not hasattr(self, "twopi_sq"):
            self.twopi_sq = self.twopi ** 2
    ...
    def forward(self, data: Dict[str, torch.Tensor], **kwargs):
        ...
        self._ensure_legacy_sog_attrs(data["positions"])
        ...
        for i in unique_batches:
            ...
            ene = self.ene_factor
            if isinstance(ene, torch.Tensor):
                ene = ene.to(device=pot.device, dtype=pot.dtype)
            else:
                ene = torch.tensor(float(ene), device=pot.device, dtype=pot.dtype)
            results.append(pot + ene)
