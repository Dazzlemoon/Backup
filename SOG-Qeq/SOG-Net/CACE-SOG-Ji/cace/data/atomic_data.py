...
class AtomicData(torch_geometric.data.Data):
    ...
    def from_atoms(
        ...
        if data_key is None:
            data_key = dict(default_data_key)
        else:
            merged_data_key = dict(default_data_key)
            merged_data_key.update(data_key)
            data_key = merged_data_key
        ...
        is_cace_calculator = (
            calc is not None
            and isinstance(calc.__class__.__module__, str)
            and calc.__class__.__module__.startswith("cace.calculators")
        )

        try:
            energy = atoms.info.get(data_key["energy"], None)  # eV
        except Exception:
            # Compatibility with ASE versions where energy may live in calculator results
            if data_key['energy'] == 'energy':
                energy = None if is_cace_calculator else atoms.get_potential_energy()
            else:
                energy = None
        if energy is None and data_key['energy'] == 'energy':
            # Some ASE readers store energy in SinglePointCalculator, not atoms.info
            energy = None if is_cace_calculator else atoms.get_potential_energy()
        ...
        try:
            ...
            if data_key['forces'] == 'forces':
                forces = None if is_cace_calculator else atoms.get_forces()
            else:
                forces = None
