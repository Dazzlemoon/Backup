...
class CACECalculator(Calculator):
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        # prepare data
        # NOTE:
        # AtomicData.from_atoms() may fallback to atoms.get_potential_energy()/get_forces()
        # when labels are missing. During ASE inference this would re-enter this calculator
        # and cause infinite recursion. Inject placeholders on a copy to avoid that path.
        ...
        data_loader = torch_geometric.dataloader.DataLoader(
            dataset=[
                AtomicData.from_atoms(
                    atoms_for_graph, cutoff=self.cutoff
                )
            ],
            batch_size=1,
            shuffle=False,
...
class CACECalculator1(Calculator):
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        # prepare data
        # Avoid recursive ASE calls when AtomicData.from_atoms falls back to
        # atoms.get_potential_energy()/get_forces() on missing labels.
        atoms_for_graph = atoms.copy()
        if "energy" not in atoms_for_graph.info:
            atoms_for_graph.info["energy"] = 0.0
        if "forces" not in atoms_for_graph.arrays:
