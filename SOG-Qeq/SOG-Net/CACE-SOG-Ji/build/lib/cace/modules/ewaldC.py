...
class SOGPotential(nn.Module):
    def __init__(self,
        ...
        self.shift_1 = torch.nn.Parameter(torch.linspace(-1, 1.0, self.bandwidth_num, dtype=torch.float32))
        # shift_1 = torch.linspace(-0.5, 1.0, self.bandwidth_num, dtype=torch.float32)
        # self.register_buffer('shift_1', shift_1)
        
        self.amplitude_1 = torch.nn.Parameter(torch.ones(self.bandwidth_num, dtype=torch.float32))
        self.Periodic = Periodic
        #print("shift_begin:",self.shift_1)

        self.norm_factor = torch.nn.Parameter(torch.tensor(1.0))
    ...
    def compute_potential_SOG(self, r_raw, q, box, compute_field=False):
        ...
        kx = torch.fft.fftfreq(self.NpointsMesh, d=Lx / (2 * np.pi)/self.NpointsMesh, device=device, dtype=torch.float32)
        ...
        ky = torch.fft.fftfreq(self.NpointsMesh, d=Ly / (2 * np.pi)/self.NpointsMesh, device=device, dtype=torch.float32)
        
        #kz = torch.linspace(-(self.NpointsMesh // 2), self.NpointsMesh // 2, self.NpointsMesh, dtype=torch.float64, device=device) * (2 * np.pi / Lz)
        kz = torch.fft.fftfreq(self.NpointsMesh, d=Lz / (2 * np.pi)/self.NpointsMesh, device=device, dtype=torch.float32)
    ...
    def compute_potential_SOG_Ewald(self, r_raw, q, box, compute_field=False):
        ...
        Nk = self.NpointsMesh//2
        n1 = torch.arange(-Nk, Nk + 1, device=device)
        n2 = torch.arange(-Nk, Nk + 1, device=device)
        n3 = torch.arange(-Nk, Nk + 1, device=device)
