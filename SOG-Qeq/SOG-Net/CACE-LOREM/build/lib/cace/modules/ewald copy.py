...
class SOGPotential(nn.Module):
    def __init__(self,
        ...
        self.shift_1 = torch.nn.Parameter(torch.linspace(0.0, 1.8, self.bandwidth_num, dtype=torch.float64))
        self.amplitude_1 = torch.nn.Parameter(torch.ones(self.bandwidth_num, dtype=torch.float64))
    ...
    def compute_potential_SOG(self, r_raw, q, box, compute_field=False):
        ...
        input_te = (2 * np.pi / L) * (wrapped_r_raw - L / 2)  # 归一化输入坐标
        ...
        for i in range(charge_complex.shape[-1]):
            ...
            #print(charge_select.shape)
            recon = pytorch_finufft.functional.finufft_type1(transposed_position, charge_select, output_shape=kx_grid.shape, eps=1e-03, isign = -1)
            #print(kx_grid.shape,recon.shape)
            # **应用 SOG 滤波**
            mult_fft = torch.complex(multiplier * recon.real, multiplier * recon.imag).to(torch.complex64)

            # **逆 FFT**
            Ifftcon = pytorch_finufft.functional.finufft_type2(transposed_position, mult_fft, eps = 1e-3, isign = 1)/ (4 * self.V* torch.pi)
            #Ifftcon = torch.fft.ifftn(mult_fft.to(torch.complex64), dim=(-3, -2, -1)).real.to(torch.float64) / (2 * self.V)
...
class EwaldPotential(nn.Module):
    ...
    def compute_potential_triclinic(self, r_raw, q, cell_now, compute_field=False):
        ...
        #print("The number of Fourier grids is ", Nk)
