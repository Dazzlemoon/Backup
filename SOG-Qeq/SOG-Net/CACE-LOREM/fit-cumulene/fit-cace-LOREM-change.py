...
MP = 2
N_dl = 2
LR_WEIGHT_SCHEDULE = [5.0, 10.0, 20.0, 20.0]
save_folder = "/data/home/public/qiuqizhi/SOG-Qeq/SOG-Net/CACE-LOREM/fit-cumulene/loss_data/CACE_LOREM_MP"+str(MP)+"_longimporve"
...
cace_nnp.to(device)
print(f"Stage LR weight schedule: {LR_WEIGHT_SCHEDULE}")
...
def compute_long_range_diagnostics(model, data_loader, device, lr_weight=1.0):
    ...
    mean_abs_q = sum_abs_q = 0.0
    # Recompute mean(|q|) robustly in one pass over data to avoid storing tensors.
    with torch.no_grad():
        abs_count = 0
        for batch in data_loader:
            batch = batch.to(device)
            batch_dict = batch.to_dict()
            pred_lr = model.models[1](batch_dict, training=False)
            q_abs = torch.abs(pred_lr["q"]).reshape(-1).detach()
            sum_abs_q += q_abs.sum().item()
            abs_count += q_abs.numel()
    mean_abs_q = sum_abs_q / max(abs_count, 1)
...
diag_csv_path = save_folder + 'diagnostics_' + time_name + '.csv'
