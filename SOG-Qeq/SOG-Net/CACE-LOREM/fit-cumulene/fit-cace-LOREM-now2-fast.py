...
save_folder = "/data/home/public/qiuqizhi/SOG-Qeq/SOG-Net/CACE-LOREM/fit-cumulene/loss_data/CACE_LOREM_MP"+str(MP)+"_now2_fast"
...
def compute_long_range_diagnostics(model, data_loader, device, lr_weight=1.0):
    ...
    return std_q, mean_abs_q, std_q_dipole, std_q_quadrupole, f_long_ratio, e_long_ratio
...
with open(diag_csv_path, 'w', newline='') as file:
    ...
    writer.writerow([
        "global_epoch", "stage", "stage_epoch", "lr_weight",
        "std_q", "mean_abs_q", "std_q_dipole", "std_q_quadrupole",
...
for stage_idx, (epochs_now, energy_weight_now) in enumerate(stage_plan, start=1):
    ...
    while stage_epoch < epochs_now:
        ...
        if should_diag:
            std_q, mean_abs_q, std_q_dipole, std_q_quadrupole, f_long_ratio, e_long_ratio = compute_long_range_diagnostics(
            ...
            print(
                ...
                f"std(q_dip)={std_q_dipole:.6f} std(q_quad)={std_q_quadrupole:.6f} "
                f"||F_long||/||F_total||={f_long_ratio:.6f} E_long/E_total={e_long_ratio:.6f}"
            )
            with open(diag_csv_path, 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([
                    global_epoch, stage_idx, stage_epoch, lr_weight_now,
                    std_q, mean_abs_q, std_q_dipole, std_q_quadrupole,
