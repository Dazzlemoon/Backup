...
save_folder = "/data/home/public/qiuqizhi/SOG-Qeq/SOG-Net/CACE-LOREM/fit-cumulene2/loss_data/CACE_LOREM_MP"+str(MP)+"_ABtry_lr_only_long300"
DEFAULT_MAX_EPOCHS = 300
DEFAULT_START_LR = 3e-3
DEFAULT_MIN_LR = 1e-6
DEFAULT_START_DECAY_AFTER = 50
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Resume training from a saved TrainingTask checkpoint.",
    )
    parser.add_argument(
        "--resume-checkpoint-path",
        type=str,
        default="checkpoint.pt",
        help="Checkpoint file path (relative to save_folder or absolute).",
    )
...
if args.resume_from_checkpoint:
    ckpt_path = args.resume_checkpoint_path
    if not os.path.isabs(ckpt_path):
        ckpt_path = os.path.join(save_folder, ckpt_path)
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(f"Resume checkpoint not found: {ckpt_path}")
    ckpt = torch.load(ckpt_path, map_location=device)
    task.load_state_dict(ckpt)
...
if args.sr_freeze_experiment:
    ...
    # Default training in this TRY script is LR-only ablation:
    # disable SR branch contribution and train only LR path.
    ...
    if args.quick_test:
        ...
        print(f"[LROnlyDefault] phase_plan={phase_plan}")
