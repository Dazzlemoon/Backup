...
save_folder = "/data/home/public/qiuqizhi/SOG-Qeq/SOG-Net/CACE-LOREM/fit-cumulene2/loss_data/CACE_LOREM_MP"+str(MP)+"_ABtest3"
...
if args.sr_freeze_experiment:
    ...
    # Default training uses three phases:
    #   1) joint warmup
    #   2) LR-focus (freeze SR readout + lower SR weight)
    #   3) joint fine-tune
    total_epochs = max(1, int(args.max_epochs))
    stage1_epochs = max(1, int(round(total_epochs * 0.2)))
    stage2_epochs = max(1, int(round(total_epochs * 0.6)))
    ...
    if args.quick_test:
        ...
        print(f"[Default3Phase] phase_plan={phase_plan}")
