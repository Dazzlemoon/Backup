...
save_folder = "/data/home/public/qiuqizhi/SOG-Qeq/SOG-Net/CACE-LOREM/fit-cumulene2/loss_data/CACE_LOREM_MP"+str(MP)+"_ABtest5_strict"
DEFAULT_MAX_EPOCHS = 500
DEFAULT_START_LR = 1e-3
DEFAULT_MIN_LR = 1e-6
DEFAULT_START_DECAY_AFTER = 10
DEFAULT_LR_WEIGHT = 10.0
DEFAULT_DIHEDRAL_DIAG_EVERY = 10
PHASE_GATE_CORR_MIN = 0.70
PHASE_GATE_AMP_RATIO_MIN = 0.50
PHASE_GATE_F_RATIO_MIN = 0.10


def evaluate_phase_gate(dihedral_result: dict):
    amp_ref = float(dihedral_result.get("amp_ref_rel", np.nan))
    amp_long = float(dihedral_result.get("amp_pred_long_rel", np.nan))
    corr_long = float(dihedral_result.get("corr_long_vs_ref", np.nan))
    f_ratio = float(dihedral_result.get("mean_f_long_over_total", np.nan))
    amp_ratio = amp_long / max(amp_ref, 1e-12) if np.isfinite(amp_ref) and np.isfinite(amp_long) else np.nan
    ok_corr = np.isfinite(corr_long) and corr_long >= PHASE_GATE_CORR_MIN
    ok_amp = np.isfinite(amp_ratio) and amp_ratio >= PHASE_GATE_AMP_RATIO_MIN
    ok_force = np.isfinite(f_ratio) and f_ratio >= PHASE_GATE_F_RATIO_MIN
    passed = bool(ok_corr and ok_amp and ok_force)
    return passed, amp_ratio, corr_long, f_ratio
...
def parse_args():
    ...
    parser.add_argument(
        "--dihedral-diag-every-epochs",
        type=int,
        default=DEFAULT_DIHEDRAL_DIAG_EVERY,
        help="Run dihedral-profile shape diagnostics every N global epochs.",
    )
    parser.add_argument(
        "--skip-dihedral-diag",
        action="store_true",
        help="Disable dihedral-profile diagnostics during training.",
    )
    parser.add_argument(
...
DIHEDRAL_DIAG_EVERY_EPOCHS = max(1, int(args.dihedral_diag_every_epochs))
if args.quick_test:
    save_folder = (
...
profile_path = dataset_root / "cumulene_profile.xyz"
...
def _build_graph_dict_from_atoms(atoms, cutoff_now: float, device_now: torch.device):
    ...
    loader = torch_geometric.dataloader.DataLoader(
        dataset=[AtomicData.from_atoms(atoms_for_graph, cutoff=cutoff_now)],
        batch_size=1,
        shuffle=False,
        drop_last=False,
    )
    batch = next(iter(loader)).to(device_now)
    return batch.to_dict()


def evaluate_dihedral_shape_diagnostics(
    model: torch.nn.Module,
    profile_xyz_path: Path,
    cutoff_now: float,
    device_now: torch.device,
    lr_weight: float,
):
    systems = ase_read(profile_xyz_path, index=":", format="extxyz")
    if len(systems) == 0:
        raise ValueError(f"No structures found in {profile_xyz_path}")

    # order by dihedral angle for stable curve diagnostics
    dihedrals = np.array([s.get_dihedral(0, 1, 11, 12) for s in systems], dtype=float)
    ...
    for s in systems:
        e_ref = None
        try:
            # extxyz usually stores reference energy in SinglePointCalculator.
            e_ref = float(s.get_potential_energy())
        except Exception:
            pass
        if e_ref is None:
            e_ref = float(s.info.get("energy", s.info.get("Energy", 0.0)))
        ref_abs.append(e_ref)
    ref_abs = np.array(ref_abs, dtype=float)
    ref_rel = ref_abs - ref_abs.min()
    ...
    with torch.enable_grad():
        for atoms in systems:
            batch_dict = _build_graph_dict_from_atoms(atoms, cutoff_now=cutoff_now, device_now=device_now)
            out_total = model(batch_dict, training=False)
            out_lr = model.models[1](batch_dict, training=False)

            e_total = float(out_total["CACE_energy"].detach().cpu().numpy().reshape(-1)[0])
            e_long_raw = float(out_lr["lr_energy"].detach().cpu().numpy().reshape(-1)[0])
            e_long_eff = float(lr_weight * e_long_raw)
            e_short_eff = float(e_total - e_long_eff)
            pred_total.append(e_total)
            pred_long.append(e_long_eff)
            pred_short.append(e_short_eff)

            f_total = out_total["CACE_forces"].detach().cpu().numpy().reshape(-1, 3)
            f_long = (lr_weight * out_lr["SOG_forces"]).detach().cpu().numpy().reshape(-1, 3)
            f_ratio = float(
                np.linalg.norm(f_long, axis=1).sum()
                / max(np.linalg.norm(f_total, axis=1).sum(), 1e-12)
            )
            f_long_over_total.append(f_ratio)

            q0 = out_lr.get("q_monopole", out_lr["q"][:, :1]).detach().cpu().numpy()
            q1 = out_lr.get("q_dipole", out_lr["q"][:, 1:4]).detach().cpu().numpy()
            q2 = out_lr.get("q_quadrupole", out_lr["q"][:, 4:9]).detach().cpu().numpy()
            q0_abs_mean.append(float(np.abs(q0).mean()))
            q1_norm_mean.append(float(np.linalg.norm(q1, axis=1).mean()))
            q2_norm_mean.append(float(np.linalg.norm(q2, axis=1).mean()))

    pred_total = np.array(pred_total, dtype=float)
    pred_long = np.array(pred_long, dtype=float)
    pred_short = np.array(pred_short, dtype=float)
    ...
    result = {
        "n_structures": int(len(systems)),
        "amp_ref_rel": float(ref_rel.max() - ref_rel.min()),
        "amp_pred_total_rel": float(pred_total_rel.max() - pred_total_rel.min()),
        "amp_pred_long_rel": float(pred_long_rel.max() - pred_long_rel.min()),
        "amp_pred_short_rel": float(pred_short_rel.max() - pred_short_rel.min()),
        "corr_total_vs_ref": _corrcoef_safe(pred_total_rel, ref_rel),
        "corr_long_vs_ref": _corrcoef_safe(pred_long_rel, ref_rel),
        "mean_f_long_over_total": float(np.mean(f_long_over_total)),
        "mean_q0_abs": float(np.mean(q0_abs_mean)),
        "mean_q1_norm": float(np.mean(q1_norm_mean)),
        "mean_q2_norm": float(np.mean(q2_norm_mean)),
    }
    return result
...
if args.sr_freeze_experiment:
    ...
    #   1) joint warmup
    #   2) strict LR-focus (SR weight=0.0, freeze SR readout)
    #   3-7) slower progressive SR return (0.05 -> 0.1 -> 0.2 -> 0.3 -> 0.5)
    total_epochs = max(1, int(args.max_epochs))
    stage1_epochs = max(1, int(round(total_epochs * 0.15)))  # warmup
    stage2_epochs = max(1, int(round(total_epochs * 0.45)))  # lr focus
    stage3_epochs = max(1, int(round(total_epochs * 0.08)))  # sr=0.05
    stage4_epochs = max(1, int(round(total_epochs * 0.08)))  # sr=0.1
    stage5_epochs = max(1, int(round(total_epochs * 0.10)))  # sr=0.2
    stage6_epochs = max(1, int(round(total_epochs * 0.07)))  # sr=0.3
    stage7_epochs = total_epochs - stage1_epochs - stage2_epochs - stage3_epochs - stage4_epochs - stage5_epochs - stage6_epochs
    if stage7_epochs < 1:
        deficit = 1 - stage7_epochs
        stage7_epochs = 1
        for _ in range(deficit):
            if stage6_epochs > 1:
                stage6_epochs -= 1
            elif stage5_epochs > 1:
                stage5_epochs -= 1
            elif stage4_epochs > 1:
                stage4_epochs -= 1
            elif stage3_epochs > 1:
                stage3_epochs -= 1
            elif stage2_epochs > 1:
                stage2_epochs -= 1
            elif stage1_epochs > 1:
                stage1_epochs -= 1

    if args.quick_test:
        phase_plan = [{
            "name": "quick_stage",
            "epochs": max(1, args.quick_epochs),
            "energy_weight": 1.0,
            "lr_weight": 1.0,
            "sr_weight": 1.0,
            "freeze_sr_readout": False,
        }]
        print(f"[QuickTest] phase_plan={phase_plan}")
    else:
        phase_plan = [
            {
                "name": "joint_warmup",
                ...
                "energy_weight": 0.5,
                "lr_weight": float(args.default_lr_weight),
                "sr_weight": 1.0,
                "freeze_sr_readout": False,
            },
            {
                "name": "lr_focus_strict",
            ...
            {
                "name": "joint_return_0p3",
                "epochs": stage6_epochs,
                "energy_weight": 0.5,
                "lr_weight": float(args.default_lr_weight),
                "sr_weight": 0.3,
                "freeze_sr_readout": False,
            ...
            {
                "name": "joint_return_0p5",
                "epochs": stage7_epochs,
                "energy_weight": 0.5,
                "lr_weight": float(args.default_lr_weight),
                "sr_weight": 0.5,
                "freeze_sr_readout": False,
        ...
        print(f"[Strict7Phase] phase_plan={phase_plan}")
...
dihedral_diag_csv_path = os.path.join(save_folder, 'dihedral_diagnostics_' + time_name + '.csv')
with open(dihedral_diag_csv_path, 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow([
        "global_epoch", "stage", "phase", "stage_epoch", "lr_weight", "sr_weight",
        "n_structures",
        "amp_ref_rel", "amp_pred_total_rel", "amp_pred_long_rel", "amp_pred_short_rel",
        "corr_total_vs_ref", "corr_long_vs_ref",
        "mean_f_long_over_total", "mean_q0_abs", "mean_q1_norm", "mean_q2_norm"
    ])
...
for stage_idx, phase_cfg in enumerate(phase_plan, start=1):
    ...
    while stage_epoch < epochs_now:
        ...
        should_dihedral_diag = (
            (not args.skip_dihedral_diag)
            and (
                (global_epoch % DIHEDRAL_DIAG_EVERY_EPOCHS == 0)
                or (stage_epoch == epochs_now)
            )
        )
        if should_dihedral_diag:
            try:
                d = evaluate_dihedral_shape_diagnostics(
                    model=task.model,
                    profile_xyz_path=profile_path,
                    cutoff_now=cutoff,
                    device_now=device,
                    lr_weight=lr_weight_now,
                )
                print(
                    f"[DihedralDiag] epoch={global_epoch} stage={stage_idx}.{stage_epoch} "
                    f"amp(ref/total/long/short)=({d['amp_ref_rel']:.6f}/"
                    f"{d['amp_pred_total_rel']:.6f}/{d['amp_pred_long_rel']:.6f}/"
                    f"{d['amp_pred_short_rel']:.6f}) "
                    f"corr(total,long)=({d['corr_total_vs_ref']:.6f},{d['corr_long_vs_ref']:.6f}) "
                    f"mean(F_long/F_total)={d['mean_f_long_over_total']:.6f} "
                    f"q(|q0|,|q1|,|q2|)=({d['mean_q0_abs']:.6f},"
                    f"{d['mean_q1_norm']:.6f},{d['mean_q2_norm']:.6f})"
                )
                with open(dihedral_diag_csv_path, 'a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([
                        global_epoch, stage_idx, phase_name, stage_epoch, lr_weight_now, sr_weight_now,
                        d["n_structures"],
                        d["amp_ref_rel"], d["amp_pred_total_rel"], d["amp_pred_long_rel"], d["amp_pred_short_rel"],
                        d["corr_total_vs_ref"], d["corr_long_vs_ref"],
                        d["mean_f_long_over_total"], d["mean_q0_abs"], d["mean_q1_norm"], d["mean_q2_norm"],
                    ])
                latest_dihedral_result = d
            except Exception as exc:
                print(f"[DihedralDiag][WARN] epoch={global_epoch} failed: {exc}")
    if latest_dihedral_result is not None:
        gate_passed, amp_ratio, corr_long, f_ratio = evaluate_phase_gate(latest_dihedral_result)
        print(
            f"[PhaseGate] stage={stage_idx}({phase_name}) "
            f"corr_long={corr_long:.6f} (>= {PHASE_GATE_CORR_MIN:.2f}) "
            f"amp_long/ref={amp_ratio:.6f} (>= {PHASE_GATE_AMP_RATIO_MIN:.2f}) "
            f"F_long/F_total={f_ratio:.6f} (>= {PHASE_GATE_F_RATIO_MIN:.2f}) "
            f"=> {'PASS' if gate_passed else 'FAIL'}"
        )
    else:
        print(f"[PhaseGate][WARN] stage={stage_idx}({phase_name}) no dihedral diagnostics to evaluate.")
...
print(f"Saved dihedral diagnostics to: {dihedral_diag_csv_path}")
