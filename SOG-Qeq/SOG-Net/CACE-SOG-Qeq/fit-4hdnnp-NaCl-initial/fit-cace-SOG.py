...
# coding: utf-8
"""
fit-4hdnnp-NaCl-initial：与 fit-4hdnnp-NaCl 相同流程，但 SOG 参数按 BSA（Ji 等 2026）
与论文一致地初始化，而非默认启发式。
- 不创建/不使用 SOGPotential；
- 仅用 ChargeEq 的长程能量，输出 key 为 SOG_potential；
- 在模型 to(device) 后调用 charge_eq.init_sog_from_bsa(r_cut=cutoff)。
数据使用 ../fit-4hdnnp-NaCl/NaCl.xyz。
"""
...
save_folder = os.path.join(os.path.dirname(__file__), "loss_data", "N_" + str(Fourier_node) + "_BSA_")
os.makedirs(save_folder, exist_ok=True)
now = datetime.datetime.now()
time_name = now.strftime("%Y%m%d_%H%M%S")

DATA_DIR = os.path.join(os.path.dirname(__file__))
train_path = os.path.join(DATA_DIR, "NaCl.xyz")
