...
class Metrics(nn.Module):
    ...
    def _collect_tensor(self,
        ...
        if target is not None:
            # 优先从 target 中取；若缺失，则回退到 pred（避免 KeyError 中断训练）
            if self.target_name in target:
                target_tensor = target[self.target_name].clone().detach()
            elif self.target_name in pred:
                target_tensor = pred[self.target_name].clone().detach()
            else:
                raise KeyError(
                    f"Target dict does not contain '{self.target_name}', "
                    f"and pred dict also lacks this key."
                )
