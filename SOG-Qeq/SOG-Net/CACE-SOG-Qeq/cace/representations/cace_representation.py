...
class Cace(nn.Module):
    ...
    def forward(
        ...
        # 保证 downstream ChargeEq / SystemChargeFromAtomicCharges 能看到必要信息
        # 1) charge：用于构造 system_charge
        if "charge" in data and "charge" not in output:
            output["charge"] = data["charge"]
        # 2) atomic_numbers / z：用于 ChargeEq 选择元素硬度 J
        if "atomic_numbers" in data and "atomic_numbers" not in output:
            output["atomic_numbers"] = data["atomic_numbers"]
        if "z" in data and "z" not in output:
            output["z"] = data["z"]
