        ...
        while True:
            ...
            for _ in range(nat):
                ...
                if len(vals) >= 7:
                    ...
                    # pos[3] + forces[3]，无 charge 列时默认为 0（与旧 ASE xyz 兼容）
                    pos_list.append(vals[0:3])
                    forces_list.append(vals[3:6])
                    charge_list.append(0.0)
                else:
                    raise ValueError(
                        "Expect 6 columns (pos+forces) or 7 (pos+forces+charge); "
                        f"got {len(vals)} numeric columns"
                    )
...
def get_dataset_from_extxyz_with_charge(
    ...
    all_data = read_extxyz_with_charge(
        train_path, cutoff, atomic_energies, z_map
    )
