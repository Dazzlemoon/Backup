...
# coding: utf-8
"""
fit-4hdnnp-Ag-cluster：

- 使用 CACE-SOG-Qeq（CACE 表示 + ChargeEq + SOG 核）在 Ag\(_3\) 簇数据集上训练；
- 不创建/不使用 SOGPotential，仅用 ChargeEq 的长程能量（key = SOG_potential）；
- 在模型 to(device) 后用 Ji 等 2026 提出的 BSA 初始化 SOG 参数；
- 数据使用当前目录下的 Ag-cluster.xyz（extxyz + forces + charge）。
"""
...
train_path = os.path.join(DATA_DIR, "Ag-cluster.xyz")
print("reading data (extxyz + charge via cace.data.extxyz_charge)")

# Ag 簇：只有 Ag 一种元素，使用 read_extxyz_with_charge + 自定义 z_map
z_map_ag = {"Ag": 47}
...
print(f"total Ag-cluster structures in xyz: {len(all_data)}")
