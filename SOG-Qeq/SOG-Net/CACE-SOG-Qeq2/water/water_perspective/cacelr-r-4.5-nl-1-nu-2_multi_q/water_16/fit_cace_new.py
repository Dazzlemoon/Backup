...
f_name = Path.cwd().parents[0].name

num_channel = int(sys.argv[1]) 

logging.info("reading data")
collection = cace.tasks.get_dataset_from_xyz(train_path='../../data-sets/train-H2O_RPBE-D3.xyz',
                                 valid_fraction=0.05,
                                 seed=1,
                                 cutoff=cutoff,
...
task.save_model(f'{f_name}-model.pth')
cace_nnp.to(device)
...
task.save_model(f'{f_name}-model-2.pth')
cace_nnp.to(device)
...
task.save_model(f'{f_name}-model-3.pth')
...
task.save_model(f'{f_name}-model-4.pth')
