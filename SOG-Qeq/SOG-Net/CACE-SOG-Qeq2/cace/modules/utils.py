...
def compute_forces(
    ...
    if len(energy.shape) == 1:
        ...
        # 关键：如果希望后续对 forces 构造的 loss 再做 backward，
        # 那么这里的 autograd.grad 必须保留计算图。
        # training=True 通常意味着 create_graph=True，但上层调用链中 training 可能未正确透传，
        # 因此这里用 create_graph 推断 retain_graph，避免 “backward second time” 报错。
        create_graph = training
        gradient = torch.autograd.grad(
            outputs=[energy],  # [n_graphs, ]
            inputs=[positions],  # [n_nodes, 3]
            grad_outputs=grad_outputs,
            retain_graph=create_graph,  # keep graph if we will backprop through forces
            create_graph=create_graph,  # Create graph for second derivative
        ...
        gradient = torch.stack([ 
            torch.autograd.grad(
                ...
                inputs=[positions],  # [n_nodes, 3]
                grad_outputs=grad_outputs,
                retain_graph=(create_graph or (i < num_energy - 1)),  # keep graph if needed
                create_graph=(create_graph or (i < num_energy - 1)),  # Create graph for second derivative
                allow_unused=True,  # For complete dissociation turn to true
    ...
    if len(energy.shape) == 1:
        ...
        for i in range(num_energy):
            gradient, virials = torch.autograd.grad(
                ...
                inputs=[positions, displacement],  # [n_nodes, 3]
                grad_outputs=grad_outputs,
                retain_graph=(create_graph or (i < num_energy - 1)),
                create_graph=(create_graph or (i < num_energy - 1)),
                allow_unused=True,
                )
            stress = torch.zeros_like(displacement)
            if compute_stress and virials is not None:
