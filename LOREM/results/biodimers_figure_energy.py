...
fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(textwidth, 3))
...
for i, key in enumerate(keys):
    for j, c in enumerate(classes):
        ...
        if metrics[key][c]["energy_mae"] > break_at:
            break_x.append(x)

        x = metrics[key][c]["forces_mae"]
        print(f"{key}: {c}, {x:.3f}")
...
ax1.set_ylim(restart_at, 21)  # most of the data
...
minor_ticks_every(ax1, 1, direction="y")
major_ticks_every(ax2, 1, direction="y")
minor_ticks_every(ax2, 0.1, direction="y")
